...
\NewDocumentCommand{\tooltip}{%
  ...
  \sbox\tiptext{%
    ...
    % \tcbox[colframe=black,colback=#7,size=fbox,arc=1ex,]{#8}%
      ...
      colback=Cornsilk,             % Background color (user-defined)
      drop shadow,  % Drop shadow effect
      enhanced,               % Enables advanced options
      % sharp corners=southwest,% Just to demo partial rounding (feel free to remove)
      halign=center,          % Center text
      valign=center,          % Vertically align content
      width=0.5\linewidth,   % Tooltip width (can be dynamic)
      on line,                % Render on same line as text
      before skip=1mm,        % Space before
      after skip=1mm,         % Space after
      ]{#8}
